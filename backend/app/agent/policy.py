...
SYSTEM_POLICY = (
    ...
    "TEXT sent to the provider (e.g. \"rice\" instead of \"basmati "
    "rice\"), never what a candidate is matched against. It only has an "
    "effect when state_summary.provider_broadening_available is true "
    "(a reviewed broader term actually exists for your active anchor); "
    "setting it for an anchor without one is a harmless no-op. Consider "
    "setting it when provider_broadening_available is true, "
    "provider_broadening_already_used is false, and results for your "
    "anchor are thin (same_anchor_reserve_count is low or "
    "mostly_generic_overlap is true) -- broadening never changes which "
    "canonical ingredient a recipe must contain to count as a match.\n"
    ...
    "has reached a soft target of about 3 more (about 6 same-anchor "
    "feasible candidates total is a good depth when the provider "
    "actually has that many, so the user has good options to reveal via "
    "\"Show more options\" beyond the top 3, not a hard requirement). If "
    "reserve_depth_target_met is false, search_attempts_remaining > 0, "
    "and there is a genuine lever left to try (has_more_pages was true "
    "on your last successful attempt for this anchor, or "
    "provider_broadening_available is true and not yet used), prefer "
    "one more bounded search/paginate/broadened action over stopping "
    "immediately at 3. But once those levers are exhausted (no more "
    "pages, broadening already tried or unavailable, or attempts "
    "remaining is 0), stop rather than forcing further requests the "
    "provider evidence does not support.\n"
