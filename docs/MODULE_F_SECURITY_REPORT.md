...
- ~~`pip-audit` PYSEC-2026-1845 (pytest)~~ — **remediated.** `pytest`/`pytest-asyncio` upgraded to `>=9.0.3,<10` / `>=1.4,<2` (see `docs/MASTER_REMEDIATION_REGISTER.md` MR-001, `RE_VERIFIED`). The advisory's vulnerable temp-directory functionality is genuinely exercised by this test suite (an earlier version of this report understated that, based on a scoping error in the original audit's `grep`) — real exposure was nonetheless low, since it requires a second unprivileged local user on the same host, which the single-tenant competition VM topology doesn't have. Fixed regardless: the upgrade is verified zero-regression (650/650 tests, no transitive churn beyond the two intended packages).
...
Nothing in this ticket was unilaterally marked as Founder-accepted risk — per `docs/SECURITY_ACCEPTANCE_MATRIX.md` Section 4, Claude Code does not have that authority. The one dependency finding raised during this ticket (MR-001, pytest advisory) was not accepted as risk; it was fixed, on explicit Founder direction, after an independent-review pass corrected the original exposure assessment. It is recorded as `RE_VERIFIED` in `docs/MASTER_REMEDIATION_REGISTER.md`. No items remain open or accepted-as-risk from Module F's security review.
