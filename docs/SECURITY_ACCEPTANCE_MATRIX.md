...
| Dependencies | Vulnerable dependency | REQUIRED/APPLICABLE | minimal deps + scanning | BUSINESS_PATH_IMPLEMENTED | `pip-audit` (backend) and `npm audit` (frontend), wired as the `security` CI job | One documented, unfixed-without-major-bump finding pending Founder decision (pytest PYSEC-2026-1845, dev/CI-only exposure) — see `docs/MODULE_F_SECURITY_REPORT.md` and `docs/MASTER_REMEDIATION_REGISTER.md`; new vulnerabilities may still appear over time |
