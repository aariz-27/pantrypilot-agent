...
| FR-11 | Reference prices are read from local SQLite | Master Blueprint | Price Repository | PP-003 | backend/tests/unit/test_price_repository.py | TBD (PR pending) | branch `feature/pp-003-pricing-cost-engine`, not yet merged | IN_REVIEW |
| FR-12 | Missing-item purchase cost is deterministic | Master Blueprint | Cost Engine | PP-003 | backend/tests/unit/test_cost_engine.py | TBD (PR pending) | branch `feature/pp-003-pricing-cost-engine`, not yet merged | IN_REVIEW |
...
| AR-07 | SQLite owns local reference/runtime data | DEC-008 | Repositories / DB | PP-003 | backend/tests/unit/test_price_repository.py | TBD (PR pending) | branch `feature/pp-003-pricing-cost-engine`, not yet merged | IN_REVIEW |
